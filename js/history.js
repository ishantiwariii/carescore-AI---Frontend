...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("filterSelect").addEventListener("change", filterReports);
});

let allReports = []; // Store data locally for fast filtering
...
function checkAuth() {
    ...
    // Setup Nav User (Optional)
    const navUser = document.getElementById("navUsername");
    if(navUser) {
        navUser.style.display = "block";
        navUser.textContent = "User"; // You can store/fetch name if needed
...
async function fetchHistory() {
    ...
    try {
        ...
        if (result.success) {
            ...
            listContainer.innerHTML = '<div class="error">Failed to load history.</div>';
        }
    } catch (error) {
        console.error("Error:", error);
        listContainer.innerHTML = '<div class="error">Server error. Ensure backend is running.</div>';
    }
}
...
function renderReports(reports) {
    ...
    if (reports.length === 0) {
        listContainer.innerHTML = '<div class="empty-state">No reports found. Upload one to get started!</div>';
        return;
    }
...
function filterReports() {
    ...
    const filtered = allReports.filter(report => {
        ...
        // Since we don't have explicit "type" in DB yet, we search in the extracted data string
        const rawText = JSON.stringify(report.raw_data || "").toLowerCase();
        
        const matchesSearch = date.includes(query) || rawText.includes(query);
        const matchesType = filterType === "all" ? true : rawText.includes(filterType);
